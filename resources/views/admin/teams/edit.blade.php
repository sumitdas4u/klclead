    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.teams.update", [$team->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lead_status_group') ? 'is-invalid' : '' }}" name="lead_status_group_id" id="lead_status_group_id">
                    @foreach($lead_status_groups as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lead_status_group_id') ? old('lead_status_group_id') : $team->lead_status_group->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact_no') ? 'is-invalid' : '' }}" type="text" name="contact_no" id="contact_no" value="{{ old('contact_no', $team->contact_no) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact_person') ? 'is-invalid' : '' }}" type="text" name="contact_person" id="contact_person" value="{{ old('contact_person', $team->contact_person) }}">
