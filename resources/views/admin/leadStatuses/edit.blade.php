    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.lead-statuses.update", [$leadStatus->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="name">{{ trans('cruds.leadStatus.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $leadStatus->name) }}">
                @if($errors->has('name'))
